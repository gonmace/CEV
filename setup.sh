...
# ── Integraciones específicas de CEV ───────────────────────────────────────────
echo "━━━ Integraciones de CEV (opcional — Enter para omitir/mantener) ━━━"
echo ""

_DEFAULT=$(get_env GOOGLE_CLIENT_ID)
read -p "Google OAuth Client ID [${_DEFAULT}]: " GOOGLE_CLIENT_ID
GOOGLE_CLIENT_ID=${GOOGLE_CLIENT_ID:-${_DEFAULT}}

_DEFAULT=$(get_env GOOGLE_CLIENT_SECRET)
read -sp "Google OAuth Client Secret (Enter para mantener actual): " GOOGLE_CLIENT_SECRET_NEW; echo
GOOGLE_CLIENT_SECRET=${GOOGLE_CLIENT_SECRET_NEW:-${_DEFAULT}}

_DEFAULT=$(get_env GOOGLE_MAPS_API_KEY)
read -p "Google Maps API Key [${_DEFAULT}]: " GOOGLE_MAPS_API_KEY
GOOGLE_MAPS_API_KEY=${GOOGLE_MAPS_API_KEY:-${_DEFAULT}}

_DEFAULT=$(get_env SITE_LOGO_URL)
read -p "URL del logo del sitio [${_DEFAULT}]: " SITE_LOGO_URL
SITE_LOGO_URL=${SITE_LOGO_URL:-${_DEFAULT}}

_DEFAULT=$(get_env N8N_BASE_URL)
if [ "${ENV_TYPE}" = "dev" ]; then
    _N8N_BASE_SUGGESTED="http://localhost:${N8N_PORT}"
else
    _N8N_BASE_SUGGESTED="https://${N8N_DOMAIN:-n8n.${DOMAIN}}"
fi
read -p "N8N_BASE_URL, base de los webhooks de n8n [${_DEFAULT:-${_N8N_BASE_SUGGESTED}}]: " N8N_BASE_URL
N8N_BASE_URL=${N8N_BASE_URL:-${_DEFAULT:-${_N8N_BASE_SUGGESTED}}}
echo ""
...
lines = [
    ...
    "",
    "# ── Integraciones CEV ────────────────────────────────────────────────────",
    kv("GOOGLE_CLIENT_ID", os.environ.get("GOOGLE_CLIENT_ID", "")),
    kv("GOOGLE_CLIENT_SECRET", os.environ.get("GOOGLE_CLIENT_SECRET", "")),
    kv("GOOGLE_MAPS_API_KEY", os.environ.get("GOOGLE_MAPS_API_KEY", "")),
    kv("SITE_LOGO_URL", os.environ.get("SITE_LOGO_URL", "")),
    kv("N8N_BASE_URL", os.environ.get("N8N_BASE_URL", "")),
    "",
]
